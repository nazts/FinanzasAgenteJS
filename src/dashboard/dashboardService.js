...
// ═════════════════════════════════════════════════════════════════════════════
// 6) BEHAVIORAL INTELLIGENCE METRICS
// ═════════════════════════════════════════════════════════════════════════════

export function getBehavioralMetrics() {
  // All profiles with behavioral data
  const profiles = db().prepare(`
    SELECT fp.*, u.username, u.first_name
    FROM financial_profiles fp
    JOIN users u ON u.id = fp.user_id
    WHERE fp.onboarding_completed = 1 AND fp.salary > 0
  `).all();

  let totalDrift = 0;
  let driftingCount = 0;
  const usersWithSpikes = [];
  const categoryIncrements = {};

  for (const p of profiles) {
    const drift = p.monthly_deviation_score || 0;
    totalDrift += drift;

    if (drift > 0.3) driftingCount++;

    // Parse recurring spikes
    let spikes = [];
    try { spikes = JSON.parse(p.recurring_spike_pattern || '[]'); } catch { /* ignore */ }
    if (spikes.length > 0) {
      usersWithSpikes.push({
        username: p.username,
        first_name: p.first_name,
        riskLevel: p.behavioral_risk_level || 'normal',
        spikeCount: spikes.length,
        categories: spikes.map(s => s.label || s.category).join(', '),
      });
    }

    // Parse category trends for aggregation
    let trends = {};
    try { trends = JSON.parse(p.category_trends || '{}'); } catch { /* ignore */ }
    for (const [cat, entries] of Object.entries(trends)) {
      if (!Array.isArray(entries) || entries.length < 2) continue;
      const last = entries[entries.length - 1];
      if (!categoryIncrements[cat]) categoryIncrements[cat] = [];
      categoryIncrements[cat].push(last.growthPct || 0);
    }
  }

  const profileCount = profiles.length || 1;
  const avgDrift = Math.round((totalDrift / profileCount) * 100);
  const pctDrifting = Math.round((driftingCount / profileCount) * 100);

  // Average growth per category
  const avgCategoryGrowth = {};
  for (const [cat, values] of Object.entries(categoryIncrements)) {
    const label = cat === 'necesidad' ? 'Necesidades' : cat === 'gusto' ? 'Ocio' : cat === 'ahorro' ? 'Ahorro' : cat;
    avgCategoryGrowth[label] = values.length > 0
      ? Math.round((values.reduce((s, v) => s + v, 0) / values.length) * 10) / 10
      : 0;
  }

  // Global savings trend (monthly savings from transactions, last 6 months)
  ...
  const riskDistribution = { normal: 0, bajo: 0, moderado: 0, alto: 0 };
  for (const p of profiles) {
    const rl = p.behavioral_risk_level || 'normal';
  ...
  return {
    ...
    usersWithSpikes,
    spikeCount: usersWithSpikes.length,
    avgDrift,
    pctDrifting,
    avgCategoryGrowth,
    savingsTrend,
